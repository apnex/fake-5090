...
| [F47](F47-per-poll-deadbus-marker-coverage-gap.md) | **Per-poll dead-bus marker coverage gap / counterproductive early marker** — a GPU-lost marker is honored by only a SUBSET of GSP-bootstrap poll engines, so freeing a stuck re-open worker by setting one marker is per-poll whack-a-mole; and setting the lock-free `os_pci` marker EARLY is counterproductive (pre-empts the `DETECTOR_MMIO_DEAD` self-heal that sets `PDB_PROP_GPU_IS_LOST`, the marker `_kgspRpcRecvPoll` honors). A13's `os_pci` cleared the lockdown poll by *accident* (`0xFFFFFFFF≠0`) but the wedge MOVED into `_kgspRpcRecvPoll` → the [[F20]] heartbeat-timeout storm. Generalises [[F44]] (one poll) to all 13 reachable GSP poll-sites. | **C7** (read-only `osIsGpuBusLost` poll-reader at the 2 GSP engine chokepoints `timeoutCondWait`/`_kgspRpcRecvPoll` + 3 hand-rolled loops + C7-e7 guard widenings) + A13′ + A14 — **BUILT + LIVE-VALIDATED 2026-06-13 (apnex.32): A14 gate 13 ms refusal; C7 contained the roll at BOTH loglevels, zero storm (GAP-4: storm removed at source)** | **field-bug** (live host hard-wedge 2026-06-06, A13 apnex.31; capture `netcon3`) | Chip-state `userspace-recovered + WPR2=0 + lockdown-passes-into-RPC` + re-open + AER injection |
| [F48](F48-pbi-capwalk-config-spin.md) | **Probe-time PBI capability-walk infinite spin on a disconnected pci_dev** — `pci_pbi.c:88` walks the PCI capability list via CONFIG-SPACE reads with no TTL/0xFF-terminator; on a stale `pci_dev_is_disconnected` device every read returns `0xFF` → unbounded 1-CPU busy-spin, unkillable modprobe holding the device lock (host SURVIVES — not F44). Config space = a **third I/O class** covered by neither the os.c MMIO short-circuit nor C7's poll table; reachable only now that the #292 stack survives to probe a stale marked device. Cross-refs [[F47]] [[F04]] [[F09]]. | *(fix pending)*: TTL-bound the walk + 0xFF terminator; `nv_pci_probe` early-bail on `pci_dev_is_disconnected`; consider `osPciRead*` class guard | **field-bug** (live 2026-06-13, apnex.32 campaign cycle-3; sysrq-l NMI capture; reboot to clear) | Stale disconnected pci_dev + driver re-bind WITHOUT re-enumeration (no slot-cycle) |
...
| C7 inflight-deadbus-poll-coverage | **[F47]** — a read-only `osIsGpuBusLost` predicate taught to the 2 GSP poll engines (`timeoutCondWait`, `_kgspRpcRecvPoll`) + 3 hand-rolled loops, so the lock-free `os_pci` marker A13 already sets is HONORED at every reachable GSP bring-up poll-site (closes the per-poll coverage gap A13 exposed live, where A10's marker covered the lockdown poll but the wedge moved into `_kgspRpcRecvPoll`). Read-only ⇒ no lock, no PM-write, pure-FALSE no-op on a live bus. Ships with A13′ (marker source + dynpower funnel) + A14 (fix-bar1 fail-fast gate). Designed apnex.32, pending impl. |
