...
- [[F40]] — `RmShutdownAdapter` destructive-teardown wedge on chip-responsive incompletely-initialized state. F40 is the downstream complication when F41's userspace recovery is used — the recovery sequence leaves the chip in a state F40 wedges on. F41 + persistence engagement (from F40's prevention) is the current operator-mitigation pair.
