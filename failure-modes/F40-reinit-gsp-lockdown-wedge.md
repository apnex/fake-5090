# F40 — chip re-init wedge on userspace-recovered eGPU (open-arm GSP lockdown-release stall)

> **Scope (2026-05-30 split — was a "two-arm" entry, now open-arm only).** F40 is the **open-arm re-init wedge ONLY**: a chip that is *dead for init* on a userspace-recovered substrate, where `RmInitAdapter` stalls on the GSP secure-boot lockdown-release — the genuine host wedge (n=13 reboots; structurally closed by **A6**). The original "`RmShutdownAdapter` destructive-teardown wedge" title was a historical misnomer (the disproved 2026-05-29 morning hypothesis); the file was renamed off that slug.
>
> The former **"F40-shutdown arm" has been reclassified out of F40** because it is not a wedge:
> - **[[F43]]** — `rm_shutdown_adapter`'s ~600 ms is a normal GSP RM-unload RPC. The "hangs every teardown" reading was *iatrogenic* (A7's old 200 ms budget guillotining a healthy call; budget is now 1200 ms). A timing-contract entry, not a disease.
> - **[[F42]]** — the genuine residual risk on the shutdown path is the leaked-worker UAF if `rmmod` races the in-flight worker (A7's SH-3 `flush_work` guard defends it; A6 has no such guard).
>
> The historical investigation log at the bottom (including the disproved shutdown-teardown framing and the interim "two-arm" reading) is preserved verbatim for traceability — treat anything below the "Historical investigation log" header as superseded provenance, not current understanding.

> **⚡ UPDATE 2026-06-04 — H-OA1 vs H-OA2 split + apnex.29 budget + the structural verdict.**
> 1. **A6 closes only the H-OA1 limb.** The "structurally closed by A6" claim above is true for
>    *one* entry — `nvidia_open`'s foreground `/dev/nvidia0` open (H-OA1). The **same re-init wedge has
>    other UN-bounded entry points (H-OA2)** — `nvidia_dev_get`/`nvidia_dev_get_uuid` (modeset/UVM/P2P,
>    nv.c:5416/5530), the deferred-open path, probe, and a **second RM bootstrap family** (power-resume,
>    `rm_power_management`/`rm_transition_dynamic_power`). See **[[F46]]** for the H-OA2 gap; the fix is
>    the **A12 complete funnel** (design-of-record
>    `nvidia-driver-injector/docs/missions/.../design/A12-init-funnel-design-of-record-2026-06-04.md`).
> 2. **Budget retuned (apnex.29).** The "200 ms / ~200 ms timeout" figures below are SUPERSEDED: a
>    healthy *full cold* init is ~1.3–1.9 s (n=16 warm + n=1 cold-silicon), so 200 ms dead-bused a
>    healthy cold open. `NVreg_TbEgpuOpenTimeoutMs` 200→**3000**, `NVreg_TbEgpuOpenGraceMs` 50→**2000**
>    (shipped apnex.29; see `finding-2026-06-04-a6-open-budget-vs-healthy-cold-init.md`). A10-v2's
>    completion-state discriminator distinguishes slow-healthy from stuck.
> 3. **Structural verdict (4-pass adversarial design).** The host *wedge* is perfectly closeable
>    in-driver (A12); *instant* termination of a genuinely-stuck non-lockdown stall is NOT (closed-RM:
>    the dead-bus marker covers only the lockdown poll; resume holds the RM API lock; a timeout ≠ proof
>    of loss). So `flush_work`/grace are load-bearing and the **sole residual is a bounded recovery
>    latency (≤ RM gpuTimeout), not a wedge — upstream-RM to make instant.**

| Field | Value |
|---|---|
| **Sources** | **A6** (F40b Tier 2 bounded-wait wrapper that structurally closes the open-arm re-init wedge → deterministic `-EIO`); C5 (sink primitive A6's timeout drives); A4 (close-path telemetry that bounded the investigation). *(The shutdown-path A7 wrapper + its ~600 ms RPC latency is [[F43]]; the leaked-worker UAF is [[F42]].)* |
| **Confidence** | field-bug (n=13+ open-arm wedge reproductions during 2026-05-29); structurally closed via A6 at n=2 validation (F40B-TEST runs 19:48 and 19:50, deterministic `-EIO` at ~200 ms). **2026-05-31 Lane 2 PMU sampling (n=4)** pinned the site to the GSP lockdown-release busy-poll (`kgspBootstrap_GH100 → gpuTimeoutCondWait(_kgspLockdownReleasedOrFmcError)`) — see §Current understanding. |
| **Predecessor evidence** | injector repo `docs/missions/mission-1-egpu-hot-plug-hot-power/experiments/h1-userspace-recovery-2026-05-28.md`; forensic archives `/var/log/mission-1-archaeology/wedge-2026-05-28-21-54/` through `/var/log/mission-1-archaeology/tbv2n2-wedge-2026-05-29/` (13 wedge boots, all OPEN arm); `/var/log/mission-1-archaeology/a7-deploy-wedge-2026-05-29/FORENSICS-REPORT.md` (the a7-deploy event that triggered the A7 work; **later reattributed** by the SH-series — A7 was not in that build and `rm_shutdown_adapter` does not hang, see [[F43]]) |
...
**2026-05-31 — Lane 2 PMU confirmation (refines points 3–4 above).** The open-arm wedge SITE is now pinned (PMU sampling, n=4, D0-pinned contained fire): the worker is in `nv_open_device → rm_init_adapter → RmInitAdapter → kgspInitRm_IMPL → kgspBootstrap_GH100 → gpuTimeoutCondWait(_kgspLockdownReleasedOrFmcError)` — **the GSP never releases secure-boot lockdown after the FSP boot commands** (a *pre-RPC* sub-wait, earlier than the `GSP_INIT_DONE` RPC). The contained fire showed **NO AER/CTO** (device + bridge `UESta`/`CESta` = 0): the chip *answers* the polled lockdown-status reads ("still locked"). So point 3's "chip doesn't complete the MMIO → CTO" is refined — within A6's 200 ms window it is a **graceful busy-poll holding the GPU group lock**, not a hung read; the `UESta=0x4000` CTO seen historically (Test B v2) is a *later* chip-drop that A6's 200 ms pre-empts. The host-lock mechanism is therefore the **multi-second lock-holding busy-poll**, not the CTO race per se. Full forensics: `nvidia-driver-injector/docs/missions/mission-1-egpu-hot-plug-hot-power/open-arm-forensics-ledger.md` (Lane 2). A6 bounds this on the open path — **except the first open of a bind** (its gate `is_external_gpu` is set lazily inside that open's `RmInitAdapter`, `osinit.c:1301`), a *patch-coverage* boundary that wedged the host on a rebind 2026-05-31 (`experiments/OA-reset-ladder-wedge-forensics-2026-05-31.md`).

**2026-06-02 — SUBSTRATE SPLIT + A6-containment correction (→ [[F44]]).** F40 has **two distinct chip-state sub-substrates**, and A6 contains only one:
- **WPR2-already-up fast-fail** — `_kgspBootGspRm` sees WPR2 up, returns `NV_ERR_INVALID_STATE` in ~µs *before* the lockdown poll. **A6 contains this cleanly** → deterministic `-EIO` ~200 ms (R2/R3/R4's 30 contained fires, 0 wedge). This is the substrate A6/R0/C5 were validated against.
- **WPR2-clear lockdown busy-poll** — a clean LAST-CLOSE drove `WPR2→0`, but the chip is still divergent; the re-open falls through to `kgspBootstrap_GH100 → gpuTimeoutCondWait` and busy-polls for the full RM `gpuTimeout`. **A6's bounded-wait alone does NOT contain this** — the timeout branch's `rm_cleanup_gpu_lost_state` BLOCKING-acquires the RM API lock the worker holds (the C5 sink can't arm), and a second contender (AER `error_detected` / `rmmod`) closes a lock-inversion cycle → host hard-wedge. That **containment-mechanism deadlock is [[F44]]** (defense **A10**, the lock-free `os_pci_set_disconnected` arm); reproduced live 2026-06-02 (2 reboots). ⇒ **Correction:** the "A6 → deterministic `-EIO`" claim holds for the fast-fail substrate only; the lockdown substrate needs A10/F44. F40 remains the chip-side cause; F44 is the host-side containment failure.

### Shutdown arm — RECLASSIFIED (2026-05-30 split)

The former "SHUTDOWN arm" mechanism text lived here. It is reclassified: the ~600 ms `rm_shutdown_adapter` latency is a normal GSP RM-unload RPC ([[F43]]), **not** a hang, and the leaked-worker UAF on the shutdown path is [[F42]]. The original investigation text is preserved verbatim in the historical log at the bottom of this file.

### Structural close (A6 — F40b Tier 2 bounded-wait wrapper, open arm)
...
*(The shutdown-path A7 wrapper + its ~600 ms RPC latency are documented in [[F43]]; A7's SH-3 `flush_work` leaked-worker guard in [[F42]]. Removed here to keep F40 open-arm-only.)*
...
*(The shutdown-arm reproduction recipe moved to [[F43]] — it exercises a latency/budget contract, not a wedge.)*
...
- Shutdown-path latency ([[F43]]) + leaked-worker UAF ([[F42]]) — reclassified out of F40 in the 2026-05-30 split.
